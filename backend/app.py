...
class App:
	...
	def __init__(self, cli_env_ids_str: str, dry_run: bool = False) -> None:
		...
		self.environments = {e.id: e for e in environments_l}
		...
		if len(cli_env_ids_str) > 0 and cli_env_ids_str[0] == '-':
			...
			if cli_env_ids and len(cli_env_ids) > 0:
				self.environments = {k: e for k, e in self.environments.items() if k not in cli_env_ids}
		else:
			...
			if cli_env_pairs and len(cli_env_pairs) > 0:
				self.environments = {k: e for k, e in self.environments.items() if k in cli_env_pairs.keys()}
				for k, v in cli_env_pairs.items():
					if v is not None:
						if k in self.environments:
		...
		if dry_run:
			for id, e in self.environments.items():
				e.state.set_dry(True)

		logger.info(f"Resulting profiles {self.environments.keys()}")

		self.environments = {id: wrap_in_cached(e) for id, e in self.environments.items()}
